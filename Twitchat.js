...
function moduleParameterChanged(param)
{
	if(param.name == "giveLinkForOAuth") {
		local.parameters.linkForOAuth.set("https://id.twitch.tv/oauth2/authorize?response_type=token&client_id="+local.parameters.clientID.get()+"&&redirect_uri=http://localhost:3000&scope=chat%3Aread+chat%3Aedit+channel%3Amoderate+whispers%3Aread+whispers%3Aedit");
	}
...
/*	
    ...
    var clientId = "client_id=" + local.parameters.client_ID.get();
    var redirectUri = "redirect_uri=" + local.parameters.redirect_URL.get();
	...
	script.log(baseURL + "&" + clientId + "&" + redirectUri + "&" + scope + "&" + state);
